...
async function dbInsert(table: string, data: object) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/${table}`, { method: "POST", headers: { ...headers, "Prefer": "return=representation" }, body: JSON.stringify(data) });
  return res.json();
}
...
const AdminPanel = ({ cafes, onClose, onRefresh, darkMode }: { cafes: Cafe[]; onClose: () => void; onRefresh: () => void; darkMode: boolean }) => {
  ...
  const Field = ({ label, field, type = "text", placeholder = "" }: { label: string; field: string; type?: string; placeholder?: string }) => (
    <div style={{ marginBottom: "14px" }}>
      ...
      <input type={type} placeholder={placeholder} value={(form as any)[field]} onChange={e => setForm(f => ({ ...f, [field]: type === "number" ? parseFloat(e.target.value) : e.target.value }))} style={inp}/>
    </div>
  );
  ...
  return (
    <div style={{ ...bg, minHeight: "100vh", fontFamily: "'Georgia', serif" }}>
      <div style={{ position: "sticky", top: 0, zIndex: 100, backdropFilter: "blur(20px)", background: darkMode ? "rgba(12,11,9,0.95)" : "rgba(247,243,238,0.95)", borderBottom: `1px solid ${borderCol}`, padding: "16px 24px", display: "flex", alignItems: "center", gap: "12px" }}>
        ...
        <h2 style={{ margin: 0, fontSize: "18px", fontWeight: "600", flex: 1 }}>⚙️ Admin Panel</h2>
      ...
      <div style={{ padding: "24px", maxWidth: "600px", margin: "0 auto" }}>
            ...
            <div style={{ display: "grid", gap: "10px" }}>
              {[...cafes].sort((a, b) => b.score - a.score).map(c => (
                <div key={c.id} style={{ background: cardBg, border: `1px solid ${borderCol}`, borderRadius: "16px", padding: "14px 16px", display: "flex", alignItems: "center", gap: "12px" }}>
                  ...
                  <button onClick={() => handleEdit(c)} style={{ background: "rgba(212,168,83,0.12)", border: "1px solid rgba(212,168,83,0.25)", color: "#d4a853", borderRadius: "8px", padding: "6px 10px", fontSize: "12px", cursor: "pointer" }}>✏️</button>
                  <button onClick={() => handleDelete(c.id)} style={{ background: "rgba(255,80,80,0.1)", border: "1px solid rgba(255,80,80,0.2)", color: "#ff6060", borderRadius: "8px", padding: "6px 10px", fontSize: "12px", cursor: "pointer" }}>🗑️</button>
                </div>
              ))}
            </div>
          </div>
        ...
        {tab === "add" && (
          <div>
            <h3 style={{ margin: "0 0 20px", fontSize: "18px", fontWeight: "400" }}>{editId ? "✏️ Izmeni lokal" : "➕ Novi lokal"}</h3>
            <Field label="Naziv *" field="name" placeholder="npr. Kafić Sunce"/>
            ...
            <div style={{ marginBottom: "14px" }}>
              ...
              <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Kratki opis lokala..." style={{ ...inp, minHeight: "80px", resize: "vertical" }}/>
            </div>
            <Field label="Šta probati" field="must_try" placeholder="npr. Cappuccino"/>
            ...
            <div style={{ padding: "16px", background: "rgba(212,168,83,0.06)", border: "1px solid rgba(212,168,83,0.15)", borderRadius: "14px", marginBottom: "14px" }}>
              <label style={{ ...lbl, color: "#d4a853" }}>🖼️ Slike (URL linkovi)</label>
            ...
            <div style={{ padding: "16px", background: "rgba(212,168,83,0.06)", border: "1px solid rgba(212,168,83,0.15)", borderRadius: "14px", marginBottom: "14px" }}>
              ...
              {Object.keys(form.ratings).map(k => (
                <div key={k} style={{ marginBottom: "10px" }}>
                  ...
                  <input type="range" min="1" max="10" step="0.1" value={form.ratings[k]} onChange={e => setForm(f => ({ ...f, ratings: { ...f.ratings, [k]: parseFloat(e.target.value) } }))} style={{ width: "100%", accentColor: "#d4a853" }}/>
                </div>
              ))}
            </div>
            ...
            <div style={{ padding: "16px", background: "rgba(212,168,83,0.06)", border: "1px solid rgba(212,168,83,0.15)", borderRadius: "14px", marginBottom: "20px" }}>
              <label style={{ ...lbl, color: "#d4a853" }}>🏷️ Kategorije</label>
            ...
            <button onClick={handleSave} disabled={saving} style={{ width: "100%", padding: "16px", background: saved ? "linear-gradient(135deg,#4caf50,#2e7d32)" : "linear-gradient(135deg,#d4a853,#b8893a)", border: "none", color: "#0c0b09", borderRadius: "14px", fontSize: "16px", fontWeight: "700", cursor: saving ? "wait" : "pointer", fontFamily: "inherit" }}>
              {saving ? "⏳ Čuvam..." : saved ? "✓ Sačuvano!" : editId ? "💾 Sačuvaj izmene" : "➕ Dodaj lokal"}
            </button>
          </div>
        )}
      </div>
  ...
  return (
    <div>
      ...
      <div style={{ padding: "20px", background: cardBg, borderRadius: "20px", border: `1px solid ${borderCol}`, marginBottom: "16px" }}>
        <p style={{ margin: "0 0 14px", fontSize: "13px", color: subtleText }}>Ostavi svoj komentar</p>
        ...
        <textarea placeholder="Tvoj komentar..." value={form.text} onChange={e => setForm(f => ({ ...f, text: e.target.value }))} style={{ ...inp, minHeight: "70px", resize: "vertical", marginBottom: "10px" }}/>
        ...
        <button onClick={handleSubmit} disabled={submitting} style={{ width: "100%", padding: "12px", background: submitted ? "linear-gradient(135deg,#4caf50,#2e7d32)" : "linear-gradient(135deg,#d4a853,#b8893a)", border: "none", color: "#0c0b09", borderRadius: "12px", fontSize: "14px", fontWeight: "700", cursor: submitting ? "wait" : "pointer", fontFamily: "inherit" }}>
          {submitting ? "⏳ Šaljem..." : submitted ? "✓ Objavljeno!" : "Objavi komentar →"}
        </button>
      </div>
      {loading ? (
        <div style={{ padding: "20px", textAlign: "center", color: subtleText, fontSize: "13px" }}>Učitavam komentare...</div>
      ) : comments.length === 0 ? (
    ...
    return (
      <div style={{ ...bg, minHeight: "100vh", fontFamily: "'Georgia', serif" }}>
        <div style={{ position: "relative", height: "55vh", overflow: "hidden" }}>
          <img src={c.gallery?.[galleryIdx] || c.image} alt={c.name} style={{ width: "100%", height: "100%", objectFit: "cover" }}/>
          ...
          <div style={{ position: "absolute", bottom: "28px", left: "24px", right: "24px" }}>
            <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "8px" }}>
              {c.tags?.map(t => <span key={t} style={{ background: "rgba(212,168,83,0.2)", border: "1px solid rgba(212,168,83,0.4)", color: "#d4a853", borderRadius: "99px", padding: "3px 10px", fontSize: "11px" }}>{t}</span>)}
            </div>
        ...
        {c.gallery?.length > 1 && (
          <div style={{ display: "flex", justifyContent: "center", gap: "6px", padding: "16px 0 4px" }}>
            {c.gallery.map((_, i) => <button key={i} onClick={() => setGalleryIdx(i)} style={{ width: i === galleryIdx ? "24px" : "6px", height: "6px", borderRadius: "99px", border: "none", background: i === galleryIdx ? "#d4a853" : borderCol, cursor: "pointer", transition: "all 0.3s", padding: 0 }}/>)}
          </div>
        )}
      ...
      {adminPrompt && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", backdropFilter: "blur(10px)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: "24px" }}>
          <div style={{ background: darkMode ? "#1a1510" : "#fff", border: `1px solid ${borderCol}`, borderRadius: "24px", padding: "32px", width: "100%", maxWidth: "320px" }}>
            <p style={{ margin: "0 0 6px", fontSize: "11px", color: "#d4a853", letterSpacing: "2px", textTransform: "uppercase" }}>⚙️ Admin</p>
      ...
      <div id="lokali" style={{ padding: "32px 24px" }}>
        ...
        {loading ? (
          <div style={{ textAlign: "center", padding: "60px 0", color: subtleText }}>
            ...
            <p style={{ fontSize: "14px" }}>Učitavam lokale...</p>
          </div>
        ) : sorted.length === 0 ? (
          <div style={{ textAlign: "center", padding: "60px 0", color: subtleText }}>
            <p style={{ fontSize: "32px", margin: "0 0 12px" }}>🔍</p>
            <p style={{ fontSize: "14px" }}>Nema rezultata za pretragu.</p>
          </div>
        ) : (
